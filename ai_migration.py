...
class MigrationAI:
    ...
    def _extract_sql_only(self, content):
        """
        Extract and clean SQL-only content from model output.
        Removes markdown, explanations, and "Changes & Enhancements" sections.
        Ensures all statements end with semicolons.
        """
        if not content:
            return ""
        ...
        # Remove "### Changes and Enhancements" section and everything after
        content = re.split(r'###\s*Changes\s+and\s+Enhancements', content, flags=re.IGNORECASE)[0]
        
        # Remove common explanation headers
        content = re.split(r'###\s*Output|###\s*Converted', content, flags=re.IGNORECASE)[0]
        
        # Clean up whitespace
        content = content.strip()
        
        # Ensure all non-comment lines ending with content have a semicolon
        lines = content.split('\n')
        cleaned_lines = []
        ...
        for line in lines:
            ...
            # Add semicolon if line has SQL content and doesn't end with semicolon, comment, or opening paren
            if stripped and not stripped.endswith((';', '(', ',', '--', '*/')):
                # Only add semicolon if it looks like a complete statement
                if any(keyword in stripped.upper() for keyword in ['SELECT', 'INSERT', 'UPDATE', 'DELETE', 'CREATE', 'ALTER', 'DROP', 'MERGE', 'WITH', 'TRUNCATE']):
...
notebook:
  sql:
    sql_script: |
      ...
      - MANDATORY ALTER: Generate each CHECK constraint ONLY as a standalone ALTER TABLE statement appearing after the CREATE TABLE statement.
      - If a UNIQUE constraint is found in the source, append this exact comment at the end of the script: -- Databricks does not enforce UNIQUE constraints; uniqueness must be handled through ETL logic like MERGE or deduplication.
...
**Output Format:**
1. Provide the **Databricks SQL code only** (no explanations, no markdown).
2. Each SQL statement must end with a semicolon.
3. Return raw, executable SQL code.
...
"""
        result = self.call_llama(prompt)
        return self._extract_sql_only(result)
